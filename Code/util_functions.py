...
def links2subgraphs(Arow, 
                    ...
                    class_values=None, 
                    parallel=True):
    # extract local context graphs for piRNA-disease pairs
    print('Local context graph extraction begins...')
    g_list = []
    if not parallel:
        ...
        with open('/home/dell/weihang/PUL_GCN/data/subgraph.pkl', 'wb') as f:
            pickle.dump(g_list, f)

        with open('/home/dell/weihang/PUL_GCN/data/Arow.pkl', 'wb') as f:
            pickle.dump(Arow, f)
...
def subgraph_extraction_labeling(ind, Arow, Acol, h=1, sample_ratio=1.0, max_nodes_per_hop=None, 
    ...
    v += len(u_nodes)
    r = r   # transform r back to rating label
    ...
    # u,v record the correspoding edges between piRNAs and disease for subgraph
    # node_labels record the type and node index for each node in subgraph
